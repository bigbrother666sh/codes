...
在部署阶段把这个文件夹下的 rules commands agents skills hooks mcp 等自动安装到 claude code中。

要求：
- 安装动作集合在 deploy.sh 中；
- 不要作为插件安装，安装参考见下；
- 如果某些 enhance 技能或者 skill 与 claude code 原生命令重名，可以加 enhance:xxx 比如 /enhance:plan
- hook 中有一个阻止写入特殊 md 文档的，那个不要，去掉那个 hook，直接改原始文件即可
...
cp -r erules/golang/* ~/.claude/rules/
...
merge `mcp-configs/mcp-servers.json` to your `~/.claude.json`.
