 ...
 * edited files in one pass. The accumulator is cleared on read so repeated
 * Stop calls do not double-process files.
 *
 * Per-batch timeout is proportional to the number of batches so the total
 * never exceeds the Stop hook budget (90s reserved for overhead).
 */
...
function typecheckBatch(tsConfigDir, editedFiles, timeoutMs) {
  ...
  try {
    ...
    stdout = err.stdout || ''; stderr = err.stderr || ''; failed = true;
  }
  if (!failed) return;
